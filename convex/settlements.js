...
export const createSettlement = mutation({
	...
	handler: async (ctx, args) => {
		...
		if (args.groupId) {
			...
			if (
				!isMember(args.paidByUserId) ||
				!isMember(args.receivedByUserId)
			) {
...
export const getSettlementData = query({
	...
	handler: async (ctx, args) => {
		...
		if (args.entityType === "user") {
			...
			// FIXED BUG: Filter out 1-on-1 transactions involving third parties!
			const settlements = [
				...mySettlements,
				...otherUserSettlements,
			].filter(
				(s) =>
					s.receivedByUserId === me._id ||
					s.receivedByUserId === other._id,
			);
			...
			// OPTIMIZED: Replaced raw table scan with specific index query
